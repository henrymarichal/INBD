...
python main.py train INBD  $HOME_DATASET_DIR/train_inputimages_one.txt   $HOME_DATASET_DIR/train_annotations_one.txt  --validation_images $HOME_DATASET_DIR/test_inputimages.txt --validation_annotations $HOME_DATASET_DIR/test_annotations.txt  --segmentationmodel=$HOME_SEGMENTATION_MODEL  --output $HOME_RESULTADOS_MODEL_DIR --per_epoch_it 3  --downsample 4 --interpolate_ambiguous 1 --lr 0.001 --val_freq 10 --epochs 50> "$stdout_file" 2> "$stderr_file" #adjust path
